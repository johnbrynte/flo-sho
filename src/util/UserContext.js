...
export const UserProvider = ({ children }) => {
  ...
  const { result: user, trigger: auth } = useApi("post", "user/auth", token)
  ...
  return (<>
    <UserContext.Provider value={{ token, loggedIn, user, login, loginError, register, registerError, logout }}>
      {children}
